...
mod tests {
    ...
    #[test]
    // user can submit its own address
    fn enter_own_address() {
        ...
        let res = query(
            ...
            mock_env(),
            QueryMsg::GetEntry {
                entry_address: "creator".to_string(),
            },
        )
        .unwrap();
    ...
    #[test]
    // user cannot submit other addresses
    fn enter_other_address() {
        let mut deps = mock_dependencies(&coins(2, "token"));
        ...
        let msg = ExecuteMsg::EnterRaffle {
            entering_address: "everyone_else".to_string(),
        };
        let _res = execute(deps.as_mut(), mock_env(), info, msg);
        assert_eq!(_res.unwrap_err(), ContractError::WrongAddress {});
    }

    #[test]
    // admin can submit other address
    fn admin_enter_other_address() {
        let mut deps = mock_dependencies(&coins(2, "token"));
        ...
        let res = query(
            ...
            mock_env(),
            QueryMsg::GetEntry {
                entry_address: "everyone_else".to_string(),
            },
        )
        .unwrap();
